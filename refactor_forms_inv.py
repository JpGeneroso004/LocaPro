...
form_conjunto_html = """{% extends 'base.html' %}
{% block title %}{{ titulo }}{% endblock %}
...
<div class="card shadow-sm border-0 p-4 mb-5">
  <form method="post">
    ...
    <div class="mb-3">
      <label class="form-label fw-bold">Nome do Conjunto</label>
      {{ form.nome }}
      <div class="form-text">Ex: Palco Principal, Piso do Bar.</div>
    </div>
    
    <div class="mb-3">
      <label class="form-label fw-bold">Quantidade de Placas</label>
      {{ form.quantidade_placas }}
      <div class="form-text">Máximo 30 placas por conjunto.</div>
    </div>
    ...
    <div class="d-grid mt-4">
      <button type="submit" class="btn btn-warning btn-lg shadow fw-bold">SALVAR CONJUNTO</button>
    </div>
  </form>
</div>
{% endblock %}
...
with open(os.path.join(templates_dir, "form_conjunto.html"), "w", encoding="utf-8") as f:
    f.write(form_conjunto_html)

print("Forms de inventario atualizados!")
